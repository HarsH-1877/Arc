import axios from 'axios';

export interface LeetCodeUser {
  username: string;
  profile: {
    ranking: number;
    userAvatar: string;
    realName: string;
    aboutMe: string;
  };
}

export interface LeetCodeStats {
  totalSolved: number;
  easySolved: number;
  mediumSolved: number;
  hardSolved: number;
  ranking: number;
}

export interface LeetCodeTopicProgress {
  tagName: string;
  tagSlug: string;
  problemsSolved: number;
}

const LC_GRAPHQL_ENDPOINT = 'https://leetcode.com/graphql';

export class LeetCodeService {
  /**
   * Get user profile
   */
  static async getUserProfile(username: string): Promise<LeetCodeUser | null> {
    try {
      const query = `
        query getUserProfile($username: String!) {
          matchedUser(username: $username) {
            username
            profile {
              ranking
              userAvatar
              realName
              aboutMe
            }
          }
        }
      `;

      const response = await axios.post(LC_GRAPHQL_ENDPOINT, {
        query,
        variables: { username }
      });

      if (response.data.data?.matchedUser) {
        return response.data.data.matchedUser;
      }
      return null;
    } catch (error: any) {
      console.error('LeetCode getUserProfile error:', error.message);
      return null;
    }
  }

  /**
   * Get user stats
   */
  static async getUserStats(username: string): Promise<LeetCodeStats | null> {
    try {
      const query = `
        query getUserStats($username: String!) {
          matchedUser(username: $username) {
            submitStats {
              acSubmissionNum {
                difficulty
                count
              }
            }
            profile {
              ranking
              userAvatar
              realName
            }
          }
          userContestRanking(username: $username) {
            rating
          }
        }
      `;

      const response = await axios.post(LC_GRAPHQL_ENDPOINT, {
        query,
        variables: { username }
      });

      const user = response.data.data?.matchedUser;
      if (!user) return null;

      const acSubmissions = user.submitStats.acSubmissionNum;
      const easySolved = acSubmissions.find((s: any) => s.difficulty === 'Easy')?.count || 0;
      const mediumSolved = acSubmissions.find((s: any) => s.difficulty === 'Medium')?.count || 0;
      const hardSolved = acSubmissions.find((s: any) => s.difficulty === 'Hard')?.count || 0;

      // Get contest rating (rounded to integer)
      const contestRating = response.data.data?.userContestRanking?.rating
        ? Math.round(response.data.data.userContestRanking.rating)
        : 0;

      return {
        totalSolved: easySolved + mediumSolved + hardSolved,
        easySolved,
        mediumSolved,
        hardSolved,
        ranking: contestRating  // Now contains contest rating instead of global ranking
      };
    } catch (error: any) {
      console.error('LeetCode getUserStats error:', error.message);
      return null;
    }
  }

  /**
   * Get topic-wise progress
   */
  static async getTopicBreakdown(username: string): Promise<Record<string, number>> {
    try {
      const query = `
        query getUserTopics($username: String!) {
          matchedUser(username: $username) {
            tagProblemCounts {
              advanced {
                tagName
                tagSlug
                problemsSolved
              }
              intermediate {
                tagName
                tagSlug
                problemsSolved
              }
              fundamental {
                tagName
                tagSlug
                problemsSolved
              }
            }
          }
        }
      `;

      const response = await axios.post(LC_GRAPHQL_ENDPOINT, {
        query,
        variables: { username }
      });

      const tagCounts = response.data.data?.matchedUser?.tagProblemCounts;
      if (!tagCounts) return {};

      const topicBreakdown: Record<string, number> = {};

      // Combine all difficulty levels
      const allTopics = [
        ...(tagCounts.fundamental || []),
        ...(tagCounts.intermediate || []),
        ...(tagCounts.advanced || [])
      ];

      // Aggregate by topic name
      allTopics.forEach((topic: LeetCodeTopicProgress) => {
        if (topic.problemsSolved > 0) {
          topicBreakdown[topic.tagName] = (topicBreakdown[topic.tagName] || 0) + topic.problemsSolved;
        }
      });

      return topicBreakdown;
    } catch (error: any) {
      console.error('LeetCode getTopicBreakdown error:', error.message);
      return {};
    }
  }

  /**
   * Verify profile description contains token
   */
  static async verifyProfileToken(username: string, token: string): Promise<boolean> {
    try {
      const profile = await this.getUserProfile(username);
      if (!profile) return false;

      // Check if token exists in aboutMe
      return profile.profile.aboutMe?.includes(token) || false;
    } catch (error: any) {
      console.error('LeetCode verifyProfileToken error:', error.message);
      return false;
    }
  }

  /**
   * Get daily submission counts for heatmap
   * Returns a map of date string -> submission count
   */
  static async getDailySubmissions(username: string): Promise<Map<string, number>> {
    try {
      const query = `
        query userProfileCalendar($username: String!) {
          matchedUser(username: $username) {
            userCalendar {
              submissionCalendar
            }
          }
        }
      `;

      const response = await axios.post(LC_GRAPHQL_ENDPOINT, {
        query,
        variables: { username }
      });

      const calendarData = response.data.data?.matchedUser?.userCalendar?.submissionCalendar;
      if (!calendarData) return new Map();

      // Parse the calendar JSON (timestamp -> count)
      const calendar = JSON.parse(calendarData || '{}');
      const dailyCounts = new Map<string, number>();

      Object.entries(calendar).forEach(([timestamp, count]) => {
        const date = new Date(parseInt(timestamp) * 1000);
        const dateStr = date.toISOString().split('T')[0];
        dailyCounts.set(dateStr, count as number);
      });

      return dailyCounts;
    } catch (error: any) {
      console.error('LeetCode getDailySubmissions error:', error.message);
      return new Map();
    }
  }
}
