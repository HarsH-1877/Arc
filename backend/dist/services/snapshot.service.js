...
const codeforces_service_1 = require("./platforms/codeforces.service");
const leetcode_service_1 = require("./platforms/leetcode.service");
class SnapshotService {
    ...
    static async createSnapshot(data) {
        try {
            const result = await database_1.default.query(`INSERT INTO snapshots (user_id, platform, timestamp, rating, total_solved, topic_breakdown)
         VALUES ($1, $2, NOW(), $3, $4, $5)
         ON CONFLICT (user_id, platform, timestamp) DO NOTHING
         RETURNING *`, [data.user_id, data.platform, data.rating, data.total_solved, JSON.stringify(data.topic_breakdown)]);
     ...
     */
    static async createLeetCodeSnapshot(userId, username) {
        try {
            const stats = await leetcode_service_1.LeetCodeService.getUserStats(username);
            const topicBreakdown = await leetcode_service_1.LeetCodeService.getTopicBreakdown(username);
            if (!stats)
                return null;
     ...
     */
    static async getUserSnapshots(userId, platform, days = 90) {
        try {
     ...
     */
    static async getLatestSnapshot(userId, platform) {
        try {
            const result = await database_1.default.query(`SELECT * FROM snapshots
         WHERE user_id = $1 AND platform = $2
         ORDER BY timestamp DESC
         LIMIT 1`, [userId, platform]);
            return result.rows[0] || null;
        }
        catch (error) {
